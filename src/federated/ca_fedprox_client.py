...
import numpy as np
import torch
from src.model.torch_model.generators import define_G
from src.model.torch_model.code_generators import SimpleCodeGenerator
from src.model.torch_model.cnn_backbone import SimpleCNN, MNISTCNN32
from torchvision import models
from src.model.train_network import CAFedProxTrainer as Trainer
...
class Client(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        self.trainer.train(self.local_epochs, current_round=config['round'])
...
def main():
    ...
    parser.add_argument('--logdir', type=str, default='client/logs/test/', 
                        help="directory to save logs/checkpoints.")
    parser.add_argument('--gen_load_epoch', type=int, default=200)
    parser.add_argument('--server_address', type=str, default='localhost:8080')
    parser.add_argument('--batch_size', type=int, default=10)
    parser.add_argument('--num_workers', type=int, default=0)
    parser.add_argument('--class_num', type=int, default=10)
    parser.add_argument('--num_local_epochs', type=int, default=1)
    parser.add_argument('--real_percentage', type=float, default=0.9)
    ...
    if args.split_mode == 'extreme':
        args.gen_load_dir = f'src/gen_ckpt/proposed_extreme/{args.data}/'
    elif args.split_mode == 'dirichlet':
        args.gen_load_dir = f'src/gen_ckpt/proposed_dirichlet/{args.data}/'
    ...
    if args.data in ['MNIST', 'fMNIST']:
        model = MNISTCNN32()
        generator = define_G(input_nz=128,
                            output_nc=1,
                            ngf=64,
                            netG='DCGANResnet',
                            norm='batch',
                            use_dropout=False,
                            init_type='normal')
        code_generator = SimpleCodeGenerator(138, 256, 128)
    ...
    opt = vars(args)
    trainer = Trainer(DEVICE, model, generator, code_generator, train_loader, test_loader, opt)
